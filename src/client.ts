...
export interface ClientOptions {
  /**
   * Defaults to process.env['AGENTBASE_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['AGENTBASE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['AGENTBASE_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Agentbase API.
 */
export class Agentbase {
   ...
   * API Client for interfacing with the Agentbase API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['AGENTBASE_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['AGENTBASE_BASE_URL'] ?? https://api.agentbase.sh] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('AGENTBASE_BASE_URL'),
    apiKey = readEnv('AGENTBASE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.AgentbaseError(
        "The AGENTBASE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Agentbase client with an apiKey option, like new Agentbase({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.agentbase.sh`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Agentbase.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('AGENTBASE_LOG'), "process.env['AGENTBASE_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.agentbase.sh';
  }
   ...
   */
  runAgent(
    params: TopLevelAPI.RunAgentParams,
    options?: RequestOptions,
  ): APIPromise<Stream<TopLevelAPI.RunAgentResponse>> {
    ...
    return this.post('/', {
      ...
      stream: true,
    }) as APIPromise<Stream<TopLevelAPI.RunAgentResponse>>;
  }
  ...
  static Agentbase = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static AgentbaseError = Errors.AgentbaseError;
